...
const DEV_MODE = true ? DEV_API_URL : PROD_API_URL;
export const API_BASE_URL = DEV_MODE;
// import.meta.env.VITE_API_URL || "http://localhost:3000/api";

/** Redirect user to backend Google OAuth flow (login/signup). */
export const getOAuthGoogleUrl = () => `${API_BASE_URL}/auth/google`;
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.addEventListener("load", async () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          // S3 upload failed
          const errorMessage =
            xhr.responseText || `S3 upload failed with status ${xhr.status}`;
